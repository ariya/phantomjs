...
First step is to build OpenSSL static library. Go to [openssl.org/source](http://www.openssl.org/source/) and download the source package (tested with version 1.0.1). Unpack the package to a working directory, e.g. `C:\openssl`. You need to have perl already installed. You can use [ActivePerl](http://www.activestate.com/activeperl/downloads). Add perl to environment variables. Configure and build OpenSSL:

Open Visual Studio command prompt (Do not open cmd) and `cd` to `C:\openssl`.
...
**Note**: Don't build openssl in [Cygwin](http://www.cygwin.com/), [MSYS](http://www.mingw.org/wiki/MSYS) and [MinGW](http://www.mingw.org/wiki/MinGW).
